...
if [ -f "$SCRIPT_DIR/update_pid.txt" ]; then
    echo "Update service is already running."
    exit 0
fi
...
# Run the update with uvicorn
nohup python danswer/background/update.py > logs/update.log 2>&1 &
echo $! >update_pid.txt
